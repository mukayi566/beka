      ...
      <nav className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button className="px-5 py-2.5 text-sm font-medium border border-white/10 rounded-full hover:bg-white/5 transition-all">
              Sign In
            </button>
            <Link href="/dashboard" className="px-5 py-2.5 text-sm font-bold bg-primary text-on-primary rounded-full emerald-glow hover:scale-105 transition-all">
              Get Started
            </Link>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <Link href="/dashboard" className="px-8 py-4 bg-gradient-to-r from-primary to-secondary text-on-primary font-bold rounded-2xl emerald-glow hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
                  Start Earning <ArrowRight size={20} />
                </Link>
                <Link href="/dashboard" className="px-8 py-4 bg-white/5 border border-white/10 text-foreground font-bold rounded-2xl hover:bg-white/10 transition-all">
                  Find Talent
                </Link>
              </div>
            ...
            <div className="md:col-span-2 glass-card rounded-[3rem] p-12 flex flex-col justify-between min-h-[450px]">
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar pt-12">
                {['Direct Sponsorships', 'Revenue Analytics', 'Brand Matching'].map(item => (
                  <div key={item} className="px-6 py-3 rounded-2xl bg-white/5 border border-white/5 whitespace-nowrap text-sm font-bold">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="bg-primary rounded-[4rem] p-16 md:p-24 text-center relative overflow-hidden group">
            ...
            <div className="relative z-10 max-w-3xl mx-auto space-y-12">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-6">
                <button className="px-10 py-5 bg-background text-foreground font-bold rounded-3xl text-lg hover:bg-background/90 transition-all">
                  Create Creator Profile
                </button>
                <button className="px-10 py-5 bg-transparent border-2 border-on-primary text-on-primary font-bold rounded-3xl text-lg hover:bg-white/10 transition-all">
                  Register as a Brand
                </button>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-col items-center md:items-end gap-6">
            ...
            <p className="text-xs opacity-40 font-bold tracking-widest">
              © 2024 BEKA. PREMIUM AFRICAN CREATOR ECONOMY.
            </p>
          </div>
        </div>
      </footer>
