...
export default function Sidebar({ role, activeTab, onTabChange, onToggleRole }: SidebarProps) {
    ...
    return (
        <aside className="hidden md:flex flex-col w-72 bg-surface backdrop-blur-md border-r border-black/5 h-screen sticky top-0 px-6 py-10">
            ...
            <nav className="flex-1 space-y-2">
                {items.map((item) => (
                    <button
                        ...
                        className={cn(
                            "w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-200 font-bold text-sm",
                            activeTab === item.id
                                ? "bg-primary text-on-primary primary-glow"
                                : "text-foreground/40 hover:bg-black/5 hover:text-primary"
                        )}
                    >
                        {item.icon}
            ...
            <div className="mt-auto space-y-4">
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 p-4 rounded-2xl bg-surface border border-black/5 text-[10px] font-extrabold uppercase tracking-widest hover:bg-black/5 transition-all text-foreground/60"
                >
                ...
                <div className="pt-6 border-t border-black/5 flex items-center justify-between px-2">
                    ...
                    <button className="text-foreground/30 hover:text-red-500 transition-colors">
                        <LogOut size={20} />
                    </button>
                </div>
            </div>
