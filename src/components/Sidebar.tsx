...
export default function Sidebar({ role, activeTab, onTabChange, onToggleRole }: SidebarProps) {
    ...
    return (
        <aside className="hidden md:flex flex-col w-72 bg-surface/30 backdrop-blur-md border-r border-white/5 h-screen sticky top-0 px-6 py-10">
                ...
                {items.map((item) => (
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === item.id
                                ? "bg-primary text-on-primary emerald-glow"
            ...
            <div className="mt-auto space-y-4">
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 p-4 rounded-2xl bg-white/5 border border-white/10 text-xs font-bold uppercase tracking-widest hover:bg-white/10 transition-all"
                >
